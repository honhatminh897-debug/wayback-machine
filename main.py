...
async def fetch_counts_total(session: aiohttp.ClientSession, domain: str) -> int | None:
    """
    Đếm tổng snapshot cho 1 domain (bao gồm subdomain) bằng matchType=domain.
    Dùng showNumPages=true&pageSize=1 để số trang == tổng bản ghi.
    Fallback qua host/prefix nếu domain không trả kết quả.
    """
    base = "https://web.archive.org/cdx/search/cdx?showNumPages=true&pageSize=1"
    patterns = [
        f"&matchType=domain&url={domain}/*",  # domain + subdomains
        f"&matchType=host&url={domain}/*",    # chỉ đúng host
        f"&matchType=prefix&url={domain}/*",  # prefix (rộng)
    ]
    for q in patterns:
...
async def fetch_earliest_snapshot(session: aiohttp.ClientSession, domain: str, semaphore: asyncio.Semaphore) -> dict:
    """
    Lấy snapshot sớm nhất + tổng số snapshot (theo domain).
    """
        ...
        for i in range(0, len(domains), BATCH_SIZE):
            ...
            for r in batch_results:
                ...
                if age is not None and age >= ALERT_AGE_YEARS:
                    ...
                    caption = (
                        ...
                        f"🔗 snapshots_total: {r.get('snapshots_total')}"
                    )
                    try:
            ...
            if now - last_edit >= PROGRESS_THROTTLE_SEC:
                ...
                msg = (
                    ...
                    f"• Đã chạy: {elapsed}s\n"
                    "🐧 Mẹo SEO: nhiều snapshot thường dễ phục hồi trust 📈"
                )
                try:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text = (
        ...
        "• Đếm *tổng snapshot của domain* (bao gồm subdomain)\n"
        "• Log tiến trình + alert domain già 🧲\n\n"
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        ...
        "• Một cột domain/URL\n"
        "• Gửi file (≤20MB)\n"
        "• File trả về có các cột: domain, earliest_snapshot, age_years, snapshots_total",
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_document(
        ...
        filename=out_name,
        caption="✅ Hoàn tất! Báo cáo 1 sheet: domain + tuổi + tổng snapshot."
    )
